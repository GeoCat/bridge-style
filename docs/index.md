...
The library uses [Geostyler](geostyler.md) as intermediate format, and uses a two-step approach:
...
- [SLD](sld.md). This is mainly targeted at Geoserver users, so it makes use of Geoserver vendor options.

- [Mapbox GL](mapbox-gl.md).

- [Mapserver](mapserver.md) Mapfiles.

Support for YSLD and ArcGIS Pro will be implemented soon.
...
To see which QGIS symbology features are correctly converted to Geostyler (and then supported in the rest of available formats, when possible), see [here](qgis.md): 
